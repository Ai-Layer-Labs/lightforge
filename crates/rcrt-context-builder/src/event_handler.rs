...
impl EventHandler {
    ...
    async fn handle_event(&self, event: BreadcrumbEvent) -> Result<()> {
        // Process user.message.v1, tool.creation.request.v1, and tool.validation.error.v1 events
        if let Some(schema) = &event.schema_name {
            if schema == "user.message.v1" {
                ...
                info!("🐛 Processing tool validation error event");
                
                // Extract session from tags (validation errors may not have session tags)
                // They're generated by tools-runner, not user actions
                // For now, use a synthetic session for debugger
                let session_tag = event.tags
                    .as_ref()
                    .and_then(|tags| tags.iter().find(|t| t.starts_with("session:")))
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| format!("session:debugger-{}", chrono::Utc::now().timestamp()));
                
                // Assemble context for tool-debugger specialist agent
                self.assemble_and_publish_for_tool_debugger(&session_tag, event.breadcrumb_id).await?;
            }
        }
    ...
    async fn assemble_and_publish_for_tool_debugger(
        &self,
        session_tag: &str,
        trigger_id: Option<uuid::Uuid>,
    ) -> Result<()> {
        ...
        let Some(trigger) = trigger_id else {
            warn!("No trigger ID provided, skipping tool-debugger context assembly");
            return Ok(());
        };
        
        info!("🐛 Assembling context for tool-debugger specialist agent");
        
        // Load tool-debugger agent definition
        let consumer_id = "tool-debugger";
        let agent_def = load_agent_definition(consumer_id, self.vector_store.pool()).await?;
        
        info!("📋 Loaded agent definition for {}", consumer_id);
        
        // Tool-debugger needs: trigger error + validation rules + failed tool
        let mut seed_ids = vec![trigger];  // The validation error
        
        // Add configured "always" sources
        if let Some(context_sources) = &agent_def.context_sources {
            if let Some(always) = &context_sources.always {
                for source in always {
                    let nodes_result = match source.source_type.as_str() {
            ...
            // Semantic search for debugging knowledge
            if let Some(semantic) = &context_sources.semantic {
                if semantic.enabled {
                    info!("  🔍 Semantic search for debugging knowledge...");
                    
                    if let Ok(Some(trigger_bc)) = self.vector_store.get_by_id(trigger).await {
                        // Extract error messages for semantic search
                        let query_text = trigger_bc.context
        ...
        info!("✅ Collected {} sources for tool-debugger", seed_ids.len());
        
        // Convert seeds to breadcrumb nodes
        let mut breadcrumbs = Vec::new();
        for seed_id in &seed_ids {
            if let Ok(Some(row)) = self.vector_store.get_by_id(*seed_id).await {
                breadcrumbs.push(crate::graph::BreadcrumbNode {
        ...
        // Publish to tool-debugger agent
        self.publisher.publish_context(
            "tool-debugger",
            session_tag,
            Some(trigger),
        ...
        info!("✅ Published context for tool-debugger with {} breadcrumbs (~{} tokens)", 
            context.breadcrumbs.len(), token_estimate);
